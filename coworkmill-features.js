(function() {
  ...
  css.textContent = `
    ...
    .cwm-err-msg { color:#f87171; font-size:11px; margin-top:5px; display:none; font-family:sans-serif; }
    .cwm-ok-icon { position:absolute; right:12px; top:50%; transform:translateY(-50%); color:#4ade80; font-size:14px; pointer-events:none; }
  ...
  s.onload = function() {
    ...
    // 順番: 施設名, 会社名, 施設URL, 担当者名, 担当者メール
    const nameInput    = inputs[0];
    const companyInput = inputs[1];
    ...
    function validate(rule, showError) {
      ...
      if (showError || rule.input?.dataset.touched) {
    ...
    function validateAll(showErrors) {
      const ok = rules.map(r => validate(r, showErrors)).every(Boolean);
      newBtn.disabled = !ok;
      newBtn.style.opacity = ok ? '1' : '0.4';
      return ok;
    }
    ...
    rules.forEach(function(rule) {
      ...
      rule.input.addEventListener('input', function() {
        if (rule.input.dataset.touched) { validate(rule, false); validateAll(false); }
      });
    });
    ...
    newBtn.addEventListener('click', async function(e) {
      ...
      document.getElementById('cwmOk').onclick = async function() {
        ...
        const { error } = await sb.from('registrations').insert([{
          space_name: spaceName, company_name: companyName,
          website: website||null, owner_name: ownerName||null,
          contact_email: email, plan, status:'pending',
          submitted_at: new Date().toISOString()
        }]);
        if (error) {
          alert('送信に失敗しました。\n' + error.message);
          newBtn.disabled = false; newBtn.textContent = '掲載を申し込む'; return;
        }
